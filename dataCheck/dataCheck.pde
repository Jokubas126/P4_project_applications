...
float soundAngleStart = 0;    
float overallAngleChange = 0;  //Can be both positive and negative
float soundChangeStartTime = 0;      //Milliseconds
float soundChangeEndTime = 10000;    //Milliseconds
float soundXAngle;     // angle that sound should come from when facing forward
int soundYAngle = -20;    // from 90 to -40
...
void setup() {
  ...
  reader = createReader("head_direction_recording.csv");
...
void lineCheck(String line) {
  ...
  if (name.equals("time")) {
    if(int(pieces[1]) > soundChangeStartTime && int(pieces[1]) < soundChangeEndTime)
    {
      ...
      soundXAngle = soundAngleStart + ((int(pieces[1]) - soundChangeStartTime) / soundChangeEndTime) * overallAngleChange;
      System.out.println("soundXAngle:" + soundXAngle + " Applied change in degrees: " + (((int(pieces[1]) - soundChangeStartTime) / soundChangeEndTime) * overallAngleChange));
    }
    ...
    String valueString = "" + xValue;
    directionList.append(valueString); //<>//
  }
}
...
void writeToFile() {
  ...
  saveTable(table, "sound_direction_recording.csv");
}
